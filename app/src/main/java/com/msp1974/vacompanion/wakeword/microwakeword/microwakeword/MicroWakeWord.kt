...
class MicroWakeWord(
    ...
    fun processAudioFeatures(features: FloatArray): DetectionInfo {
        if (features.isEmpty())
            return (DetectionInfo(false, 0f))
        ...
        return DetectionInfo(isWakeWordDetected(probability), probability)
    }
